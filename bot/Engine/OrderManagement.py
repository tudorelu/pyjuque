...
def execute_bot(session, bot, exchange, strategy):
		...
		for pair in active_pairs:
				try_entry_order(session, bot, pair, strategy, exchange)

		# Step 3: Retreive all open orders on the bot
		print("Getting open orders:")
		open_orders = bot.getOpenOrders(session)
		print(open_orders)
		...
		for order in open_orders:
				try_exit_order(session, bot, order, exchange)

def try_entry_order(session, bot, pair, strategy, exchange):
...
def try_exit_order(session, bot, order, exchange):
